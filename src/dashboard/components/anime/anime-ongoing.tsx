...
export default function AnimeOngoing({ onAnimeSelect }: AnimeOngoingProps) {
  ...
  useEffect(() => {
    fetchOngoingData(currentPage);
  }, [currentPage]);

  const fetchOngoingData = async (page: number) => {
    try {
      setLoading(true);
      const response = await fetch(`https://api.bellonime.web.id/otakudesu/ongoing?page=${page}`);
      
      if (!response.ok) {
        throw new Error('Failed to fetch ongoing anime data');
      }
    ...
    return (
      <Card>
        <CardContent className="p-6 text-center">
          <div className="text-red-500 mb-4">Error loading ongoing anime</div>
          <p className="text-muted-foreground mb-4">{error}</p>
          <Button onClick={() => fetchOngoingData(currentPage)} variant="outline">
            Try Again
          </Button>
        </CardContent>
      </Card>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold">Currently Airing Anime</h3>
